...
Each host / module returns a table, which is then merged into the calling function, eventually leading to an entirely merged table, which is then returned by `system.lua` to `main.lua`.
`main.lua` then converts this table to json, and prints it, where `reload.sh` then reads it and forks it out to the generators.
